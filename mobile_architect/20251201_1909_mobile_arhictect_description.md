...
### **✅ COMPLETED**
1. **Laravel Mobile APIs**: Fully functional endpoints under `/api/v1/`
2. **Authentication Working**: `POST /api/v1/auth/login` returns valid Sanctum tokens
...
```
**Symptoms**: Protected mobile endpoints (`/api/v1/auth/me`) return HTML instead of JSON due to tenant resolution interference.
...
2. **Exclude Mobile Routes**: Add `/api/v1/*` to reserved/excluded routes
3. **Test Protected Endpoints**: Verify `/api/v1/auth/me` returns JSON
...
export class ApiService {
  ...
  login(credentials: LoginRequest): Observable<AuthResponse> {
    return this.http.post(`${this.baseUrl}/api/v1/auth/login`, credentials);
  }
}
```
...
## **CURRENT BLOCKER**
Platform mobile APIs (`/api/v1/auth/me`) are being processed by tenant resolution, causing HTML responses instead of JSON.

## **REQUIRED SOLUTION**
1. **Exclude `/api/v1/*` routes from tenant resolution**
...
platformLogin(credentials) → POST /api/v1/auth/login
...
**Claude CLI, help us:**
1. **Locate tenant configuration** to exclude `/api/v1/*` routes
