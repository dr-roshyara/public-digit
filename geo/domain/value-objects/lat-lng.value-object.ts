...
export class LatLng {
     ...
     * @throws DomainError with code INVALID_COORDINATES
     */
    private validateCoordinates(lat: number, lng: number): void {
        if (!LatLng.isValid(lat, lng)) {
            throw new DomainError('INVALID_COORDINATES', {
                latitude: lat,
                longitude: lng,
                validRange: '[-90 ≤ lat ≤ 90, -180 ≤ lng ≤ 180]'
            });
        }
    }
     ...
     * @throws DomainError with code INVALID_ACCURACY
     */
    private validateAccuracy(accuracy?: number): void {
        if (accuracy !== undefined && accuracy < 0) {
            throw new DomainError('INVALID_ACCURACY', {
                received: accuracy,
                requirement: 'Must be positive value in meters'
            });
        }
    }
    ...
    static fromJSON(data: LatLngJSON): LatLng {
        if (!LatLng.isLatLngJSON(data)) {
            throw new DomainError('INVALID_JSON_INPUT', {
                expected: '{ lat: number, lng: number, accuracy?: number }',
                received: data
            });
        }
    ...
    static midpoint(coordinates: LatLng[]): LatLng {
        if (!coordinates?.length) {
            throw new DomainError('EMPTY_COORDINATES', {
                message: 'Cannot calculate midpoint of empty array'
            });
        }
