...
// TEMPORARILY DISABLED: WiFi/Hybrid features for initial compilation
...
// TEMPORARILY DISABLED: All complex adapters for initial compilation
...
@NgModule({
  ...
  providers: [
    ...
    // Strategy Adapters (Core only - minimal set for compilation)
    BrowserGeoLocationAdapter,
    // TEMPORARILY DISABLED: Complex adapters for initial compilation
    // IpwhoisGeoLocationAdapter,
    // WifiPositioningService,
    // WifiContextBuilderService,
    // ManualDetectionStrategy,

    // Detection Orchestration (Temporarily disabled)
    // TEMPORARILY DISABLED: Hybrid service uses WiFi
...
// TEMPORARILY DISABLED: WiFi/Hybrid features
...
export class GeoLocationFacade {
  ...
  // TEMPORARILY DISABLED: HybridGeoLocationService
  ...
  async initGeoLocation(): Promise<void> {
    // TEMPORARILY DISABLED: Will be restored when hybrid service is fixed
    // const detected = await this.hybrid.detect();
    // this.location$.next(detected);
    console.warn('GeoLocationFacade: Hybrid detection temporarily disabled');
  }
